...
function constructUrl() {
  ...
  const customTag = encodeURIComponent(
    `Env=${VideoPreRollDFP.customTargeting.environment}&Gender=${VideoPreRollDFP.customTargeting.userGender}${ageTag}&Genres=${VideoPreRollDFP.customTargeting.gameGenres}&PlaceID=${VideoPreRollDFP.placeId}&Time=${VideoPreRollDFP.customTargeting.adTime}&PLVU=${VideoPreRollDFP.customTargeting.PLVU}`
  );
...
function correctIEModalPosition(dialog) {
  if (dialog.container.innerHeight() <= 30) {
    ...
    innerContainer.css({
      position: "relative",
      top: `${shiftDistance}px`,
    });
    dialog.container.find(".VprCloseButton").css({
      top: `${shiftDistance - 10}px`,
      "z-index": "1003",
    });
  }
}
