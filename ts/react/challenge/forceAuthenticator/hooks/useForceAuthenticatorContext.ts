...
import { ForceAuthenticatorContext } from "../store/contextProvider";
 ...
 */
const useForceAuthenticatorContext: () => ForceAuthenticatorContext = () => {
  const context = useContext(ForceAuthenticatorContext);
  if (context === null) {
    throw new Error(
      "ForceAuthenticatorContext was not provided in the current scope"
    );
  }
...
export default useForceAuthenticatorContext;
